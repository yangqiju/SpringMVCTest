define([ "dojo/_base/declare", "dojo/_base/lang", "dijit/Tree" ], function(
		declare, lang, Tree) {
	return declare("ModuleTree", Tree, {
		...
		getIconClass : function(item, /* Boolean */opened) {
			...
			if (type == "folder") {
				...
				// Lots of modules are marked as type undefined, for which we
				// have no icon, so use object instead.
		...
		onClick : function(item, nodeWidget) {
			var type = item.type;
			if (type == "folder") {
				// Since folders have no associated pages, expand the TreeNode
				// instead, to hint the user
				...
				//add url, modify at 2013.02.01 by limj
				addTabPane(item.fullname, this.version, item.url);
			}
		},

		selectAndClick : function(path) {
			// summary:
			// Helper method used from welcome screen, ex:
			// moduleTree.selectAndClick(["dojo/", "dojo/query"])
			this.set("path", [ "root" ].concat(path)).then(
